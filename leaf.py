...
filepath = (r"D:\downloads\Leaf_disease_ditection\model.h5")
...
def pred_tomato_dieas(tomato_plant):
  ...
  print(pred)
  if pred==0:
      return "Rice - Bacteria Leaf Blight",'Rice - Bacteria_blight.html'
  elif pred==1:
      return "Rice - Brown Spot",'Rice - Brown_spot.html'
  elif pred==2:
      return "Rice - Leaf Smut",'Rice - Leaf_smut.html'
  if pred==3:
...
def predict():
     if request.method == 'POST':
        ...
        file_path = os.path.join(r"D:\downloads\Leaf_disease_ditection\static\upload",filename)
        file.save(file_path)
